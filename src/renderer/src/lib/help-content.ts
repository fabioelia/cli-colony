    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Health view', detail: 'Toggle the aggregate health dashboard for all pipelines. Shows a large overall success rate percentage (green ≥80%, amber 50–79%, red <50%), a horizontal proportion bar per pipeline (green=success, red=failure, gray=skipped), and a Recent Failures list of the last 10 failures across all pipelines (name, time, error preview). A 24h/7d time-range toggle filters which runs are included. Zero-failures state shows a green checkmark. The detail table below lists name, enabled status, last fired, fire count, consecutive failures, success rate, and last error — click any row to jump to that pipeline.', icon: 'Activity' },
