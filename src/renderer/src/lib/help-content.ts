    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Colony Schedule (7d)', detail: 'Combined heatmap of all pipeline and persona scheduled fires over the next 7 days. Rows = days of the week, columns = hours (0–23). Cell intensity reflects how many items are scheduled in that slot. Cells with 4+ concurrent fires turn red — high contention risk. Hover any cell to see exactly which pipelines and personas fire at that time. Hidden when no enabled cron schedules exist.', icon: 'Calendar' },
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Playbook Memory', detail: 'Each playbook can accumulate learnings across runs. A "Memory (N lines)" button appears next to the playbook selector when a playbook is selected — click to view, edit, or clear stored memory. Memory is automatically injected into the session prompt at launch. Sessions can write new learnings by appending lines prefixed with "MEMORY:" to their output — these are captured on session exit and deduplicated. Memory is capped at 50 lines (oldest entries drop off). The memory file lives at ~/.claude-colony/playbooks/{playbook-slug}.memory.md.' },
          { label: 'Session Presets', detail: 'Save the current dialog state as a named reusable preset using "Save as Preset". Presets appear in a "Preset" dropdown at the top of the dialog — selecting one fills all fields (model, directory, agent, permission mode, effort, color, prompt). Delete a preset by clicking the × next to its name in the chip row. Presets are also available as "Launch: <name>" entries in the command palette (Cmd+K) for instant session creation without opening the dialog. Stored in ~/.claude-colony/session-presets.json.' },
          { label: 'Auto-tags', detail: 'Sessions are automatically tagged when they exit based on heuristics: "failed" (non-zero exit), "pipeline" (launched by a pipeline), "persona" (launched by a persona), "long-running" (ran > 30 min), "committed" (made ≥1 git commit during the session), "budget-exceeded" (hit per-session cost cap), "fan-out" (spawned as a child of a fan-out session), "costly" (cost > $0.50), "quick" (clean exit in < 2 min). Combined with playbook tags, auto-tags populate the tag filter chips in the sidebar automatically.' },
          { label: 'Tag Filters', detail: 'A row of clickable tag chips appears below the search bar whenever sessions have tags. Shows the top 8 most common tags with their session count. Click a chip to filter — only sessions matching any active tag are shown (OR logic). Multiple chips can be active at once. Click the × button to clear all filters. Active filters are saved to localStorage and restored on next launch. Persona tags show as "DeveloperName" (prefix stripped), pipeline tags show as "PipelineName".' },
          { label: 'Custom Tags', detail: 'Define your own auto-tag rules in Settings → Custom Tag Rules. Each rule has a tag name and a condition: cost-gt/lt (cost in $), duration-gt/lt (seconds), exit-code (exact match), dir-contains (working directory substring), name-contains (case-insensitive), or name-regex (regex on session name). Rules are evaluated alongside built-in tags when a session exits. Max 20 rules. Tag names are automatically slugified (lowercase, hyphens). Changes take effect immediately — no restart required.' },
          ...
          { label: 'Additional Directories', detail: 'Give a session access to extra repos or directories beyond the working directory. Click "+ Add" to open a folder picker — each selected directory is passed as --add-dir to the Claude CLI, expanding its file context. Shown as removable chips. Max 5 directories. When cloning a session, its additional directories are pre-filled. Saved and restored with session presets.' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Proof of Work', detail: 'Auto-generated session summary written to ~/.claude-colony/proofs/<YYYY-MM-DD>/<name>-<ts>.md on session exit. Includes: exit code, duration, cost, branch, commits, files changed, and last 30 lines of output (ANSI-stripped). Only written for sessions longer than 5 seconds. Proofs older than 14 days are pruned automatically. Click the FileText icon on any completed session to open the proof in your system text editor.', icon: 'FileText' },
          ...
          { label: 'Continue session', detail: 'Right-click an exited session → Continue to launch a new session with the full context of the previous run injected as a prompt. The new session inherits the previous session\'s model, agent, permission mode, MCP servers, effort level, and working directory. The name gets a "(cont)" suffix. Useful for picking up where a session left off, or diagnosing a failed session with full history.', icon: 'RotateCcw' },
          ...
          { label: 'Group by', detail: 'A grouping selector appears above the session history list. Group by Persona, Project (working directory), Status, Pipeline, or Space. Pipeline grouping shows sessions under their pipeline run with an aggregate status dot (green = all done, blue = running, red = failed). Non-pipeline sessions appear under "Manual Sessions". Space grouping organizes sessions across personas by task (see "Task Spaces"). Groups are collapsible and the mode persists across sessions.', icon: 'Layers' },
          { label: 'Task Spaces', detail: 'Spaces group sessions from different personas around a shared task — e.g. "Auth Refactor" or "PR Review Fixes". Create a space via the Folder icon button (visible when grouped by Space) or type a name and press Enter. Assign sessions via right-click → Move to Space. When grouped by Space, each group header shows a colored left border, a Folder icon, aggregate session count, and total cost. Pipeline-triggered sessions auto-assign to a space with a matching name (case-insensitive). Archive empty spaces via the group menu (⋯).', icon: 'Folder' },
          { label: 'Space Hub', detail: 'Click any space name header (when grouped by Space) to open the Space Hub — a project dashboard showing all sessions assigned to the space, pipeline activity (pipeline runs matching the space name), quick stats (total sessions, total cost, success rate), and recent activity events. Click ← Back or click a session row to navigate directly to that session. Rename a space by clicking its name inline. Archive via the Archive button in the hub header.', icon: 'Folder' },
    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'More menu', detail: 'Secondary actions behind the ⋯ button: AI Generate (describe a pipeline in plain English), Select mode (bulk enable/disable/run/delete), History Search (full-text search across all pipeline run history), Export/Import/Paste YAML, Reload (re-read YAML files from disk), Rules (learned review patterns from maker-checker runs), Audit (AI review of all pipeline configs with severity findings).', icon: 'MoreHorizontal' },
          { label: 'Select (bulk actions)', detail: 'Enter selection mode to bulk-manage pipelines. Click Select in the More (⋯) menu. Once 1+ pipelines are selected, a bulk action bar appears above the list with Enable, Disable, Run Now, and Delete buttons. "Select All" / "Deselect All" toggles the full visible set. Press Escape to exit selection mode.', icon: 'CheckSquare' },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Health Recovery', detail: 'Click a "degraded" or "failing" health badge to open an inline recovery panel below the pipeline card. The panel shows the last 3 debug log entries and any hook error. Three action buttons: "Reset Health" clears the consecutive failure counter back to healthy; "Retry Now" re-triggers the pipeline immediately; "Pause & Edit" disables the pipeline and opens its YAML editor for inspection. The recovery panel only appears for unhealthy pipelines — click the badge again to close it.' },
          ...
          { label: 'Session auto-naming', detail: 'Pipeline-launched sessions automatically get the pipeline name as their display name (e.g. "Pipe (Automated PR Review)") when no action.name is set in the YAML. Set action.name in the pipeline YAML to override with a custom name or a template like "Review {{pr.number}}".' },
          { label: 'Sessions badge', detail: 'Shows a count of sidebar sessions spawned by this pipeline (running + stopped). Click to focus the most recently active session. Hidden when no sessions from this pipeline exist.' },
          ...
          { label: 'Run with Options...', detail: 'Opens a dialog pre-filled from the pipeline YAML where you can override prompt, model, working directory, and budget cap for this run only — YAML is never modified. If the prompt contains Mustache template variables ({{var_name}}), a collapsible "Template Variables" section appears below the prompt with an input field per detected variable. Fill in any variable to override its runtime value for this run; empty fields fall back to the pipeline\'s normal context. Submit with Cmd+Enter or the Run button.', icon: 'Play' },
        ],
      },
      {
        ...
        items: [
          ...
          { label: 'GitHub Webhook Variables', detail: 'When source: github, Colony extracts structured template vars from the payload. All events: {{github_event}}, {{github_repo}}, {{github_sender}}. Push: {{github_branch}}, {{github_commit}}, {{github_commit_message}}, {{github_pusher}}. Pull request: {{github_action}}, {{github_pr_number}}, {{github_pr_title}}, {{github_pr_branch}}, {{github_pr_base}}, {{github_pr_author}}, {{github_pr_url}}. Issues: {{github_action}}, {{github_issue_number}}, {{github_issue_title}}, {{github_issue_author}}. Use in your prompt: "Reviewer PR #{{github_pr_number}}: {{github_pr_title}} by {{github_pr_author}}".' },
          ...
          { label: 'review-requested condition', detail: 'Only fire when you are a requested reviewer on the PR. Add condition: {type: review-requested} to a git-poll pipeline. Checks whether your GitHub username (from Colony settings) appears in the PR\'s requested_reviewers list. Once you submit a review, GitHub removes you from the list — so the condition naturally stops firing for that PR. Use with interval: 300 (5 min polling) for near-real-time review request detection without constant session launches.' },
          { label: 'issue-assigned condition', detail: 'Fire when a GitHub Issue is assigned to you. Add condition: {type: issue-assigned} to a git-poll pipeline. On each poll Colony fetches all open issues across configured repos and matches any where your GitHub username appears in assignees. Optionally add label: "ready" to filter to issues that also have a specific label (case-insensitive substring match). Dedup is per issue+updatedAt so the same issue only re-triggers when it is updated (e.g. re-assigned). Template vars available in the prompt: {{issue.number}}, {{issue.title}}, {{issue.body}}, {{issue.url}}, {{issue.labels}}, {{issue.author}}. Use the "Issue Autopilot" recipe as a starting point.' },
          { label: 'Idle Nudge', detail: 'Add idle_nudge: {after_minutes: 5, message: "You seem stuck — try a different approach.", max_nudges: 2} to any session action. When the session produces no PTY output for after_minutes, Colony automatically whispers the message via steer. Nudge count is capped at max_nudges (default 2). An activity log entry and desktop notification fire on each nudge. The interval is cleaned up when the session exits or pipelines are stopped.' },
          { label: 'on_failure config', detail: 'Add on_failure to any action to control failure behavior. Supports: notify: true (desktop notification), run: "action-name" (fire named action), retry: {max: N} (retry up to N times with previous output as context), webhook: {url, method?, headers?, body?} (POST to any HTTP endpoint — see Outbound Webhooks).' },
          { label: 'on_success config', detail: 'Add on_success to any action to fire handlers after success. Supports: notify: true (desktop notification), run: "action-name" (fire named action), chain: "pipeline-name" (trigger another pipeline), webhook: {url, method?, headers?, body?} (POST to any HTTP endpoint — see Outbound Webhooks).' },
          { label: 'Outbound Webhooks', detail: 'Add webhook: {url: "https://...", method?: "POST"|"PUT", headers?: {Authorization: "Bearer ..."}, body?: "..."} to on_success or on_failure. The body is a Mustache template with vars: {{pipeline_name}}, {{action_name}}, {{status}}, {{duration_ms}}, {{cost}}, {{run_id}}. If body is omitted, sends a JSON object with all those fields. Fires fire-and-forget with a 10s timeout. Errors are logged but do not block the pipeline. URL must start with http:// or https://. Domain only is logged (no secrets). The pipeline history records webhookFired: true when triggered.' },
          { label: 'Playbook action field', detail: 'Add playbook: "PR Review" to any pipeline action to inherit defaults from a playbook YAML in ~/.claude-colony/playbooks/. Playbook values (model, prompt, workingDirectory) are defaults — explicit action fields always win. Useful for DRY pipeline config: define a common model/agent combo once in a playbook and reference it from multiple pipelines.' },
        ],
      },
      {
        ...
        items: [
          ...
          { label: 'Re-run with Overrides', detail: 'Opens the Run with Options dialog pre-filled with the pipeline\'s current prompt, model, and budget. Change any field for this run only — overrides are one-shot and do not persist to the YAML. Available on every history row via the Wand icon. Same dialog as right-click → Run with Options on the pipeline card.', icon: 'Wand2' },
          ...
          { label: 'Execution Timeline', detail: 'Click the clock icon on any history row to expand a structured execution timeline below it. Shows: trigger event, matched conditions (PRs/commits/files), each stage with duration and cost, sessions spawned, budget stop, webhook delivery, diff stats, and final result. One row expanded at a time. Color-coded: green = success, red = failure, amber = warning, purple = trigger.', icon: 'Clock' },
          ...
          { label: 'Run Comparison', detail: 'Select two history runs via their checkboxes and click Compare to see a structured side-by-side diff — duration delta, cost delta, pass/fail change, and stage-by-stage status/timing breakdown. Earlier/Later labels are assigned automatically by timestamp. Click "Copy" in the comparison header to copy a Markdown summary to clipboard.', icon: 'ArrowUpDown' },
          { label: 'Docs', detail: 'Companion readme if <name>.readme.md exists alongside the pipeline.' },
          { label: 'Debug Log', detail: 'Per-poll trace showing condition evaluation, trigger matching, dedup decisions, action dispatch, and error details. Last 50 entries per pipeline (persisted to disk after every poll). Entries are color-coded: green = fired, amber = skipped/dedup, red = error. Click any entry to expand the full line. Use the search box to filter by keyword. Auto-refreshes when the pipeline polls. Useful for debugging why a pipeline fired (or didn\'t) without reading raw JSON files.', icon: 'List' },
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Webhook Channels', detail: 'Forward Colony notifications to Slack, Discord, or any HTTP POST endpoint. Add a channel with a name, webhook URL, and type — Colony formats the payload as Slack blocks, Discord embeds, or plain JSON. Each channel has an enabled toggle and optional event source filters (pipeline, persona, session, approval, error, all). Test button sends a sample payload to verify the webhook is working.', icon: 'Globe' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'API URL', detail: 'Base URL for the Colony REST API (http://127.0.0.1:7474/api/). Click the copy button to copy it. Use GET /api/status, /api/sessions, /api/pipelines, /api/personas, or GET /api/events (SSE stream).' },
          { label: 'API token', detail: 'Set a token to require authentication on the Colony REST API. When configured, requests must include a Bearer token or X-Colony-Token header. Leave empty for no authentication. Use ✨ Generate to create a secure 32-char token, Copy to clipboard it, and Play (▶) to test the connection to the local server.' },
          { label: 'GET /api/openapi.json', detail: 'Machine-readable OpenAPI 3.0 spec describing all Colony API endpoints. Use to auto-generate clients, import into Postman/Insomnia, or explore with curl.' },
          { label: 'GET /api/docs', detail: 'Interactive Swagger UI docs for the Colony API. Enter your token at the top and try any endpoint directly from the browser. Loads the spec from /api/openapi.json.' },
          { label: 'Dashboard — /api/dashboard', detail: 'Self-contained HTML web dashboard for monitoring Colony from any browser — phone, SSH session, or second screen. Shows running/waiting/stopped sessions with cost and idle time, enabled pipelines with next-fire times and last-run badges, active personas with run counts, and health checks. Auto-refreshes via SSE (EventSource). Token entered inline is stored in sessionStorage for the tab lifetime. No external CDN dependencies — works on air-gapped networks.' },
          { label: 'GET /api/status', detail: 'Health check — returns { ok: true, version, uptime }. Use to verify the server is running before making other API calls.' },
          { label: 'GET /api/sessions', detail: 'Returns a list of all sessions: id, name, status, cost, uptime (ms). Filter by status client-side.' },
          { label: 'POST /api/sessions', detail: 'Create a new session. Body: { "prompt"?: "...", "name"?: "...", "model"?: "claude-opus-4-7", "permissionMode"?: "autonomous|auto|supervised", "workingDirectory"?: "/path" }. Returns { id, name }. Rate-limited to 5 per minute.' },
          { label: 'GET /api/sessions/:id', detail: 'Returns full session detail by id or name.' },
          { label: 'POST /api/sessions/:id/stop', detail: 'Stop a running session. Returns { ok: true } or 404. Idempotent — safe to call on already-stopped sessions.' },
          { label: 'GET /api/sessions/:id/output', detail: 'Returns the PTY buffer content for a session as { output: "..." }. Capped at last 100KB. Works on running and stopped sessions. Output includes raw ANSI codes.' },
          { label: 'DELETE /api/sessions/:id', detail: 'Remove a stopped session from the list. Returns 409 if the session is still running — stop it first.' },
          { label: 'POST /api/sessions/:id/steer', detail: 'Send a prompt to a running session. Body: { "prompt": "your message" }. Returns { ok: true }.' },
          { label: 'POST /api/sessions/:id/whisper', detail: 'Alias for steer. Send a message to a running session. Body: { "prompt": "your message" }. Accepts id or name.' },
          { label: 'GET /api/personas', detail: 'Returns a list of all personas: id, name, enabled, model, schedule, lastRun, runCount, active. Useful for monitoring persona health from external tools.' },
          ...
          { label: 'POST /api/pipelines/:name/trigger', detail: 'Trigger a named pipeline run immediately. Optional body: { "prompt"?: "...", "model"?: "...", "workingDirectory"?: "/path", "maxBudget"?: 5.0, "vars"?: { "branch": "main" } }. vars keys override {{varName}} Mustache placeholders in the pipeline YAML. maxBudget clamped 0.01–100. Empty body triggers with pipeline defaults.' },
          { label: 'GET /api/pipelines/:name/preview', detail: 'Dry-run a pipeline: evaluate its trigger and conditions without firing. Returns { wouldFire, matches, conditionLog }. For git-poll triggers, makes live GitHub API calls. Use to validate pipeline configuration from CI/CD before merging.' },
          { label: 'POST /api/pipelines/validate', detail: 'Validate pipeline YAML structure. Body: { "yaml": "<YAML string>" }. Returns { valid, pipeline?, errors[], warnings[] }. Checks required fields, condition tree, parallel nesting, and action constraints. Does not write to disk. Always returns HTTP 200 — check the valid field.' },
          { label: 'GET /api/pipelines/:name/runs', detail: 'Returns the run history for a named pipeline. Response: { pipeline, runs: [...] } sorted newest-first. Optional ?limit=N caps results (max 20). Includes success, durationMs, totalCost, sessionIds per entry. Run entries include webhookDeliveries[] with per-attempt HTTP status, latency, and retry outcomes. Use for monitoring dashboards and failure alerting.' },
          { label: 'GET /api/pipelines/:name/runs/latest', detail: 'Returns just the most recent run entry for a pipeline. Returns 404 if the pipeline has no history. Convenient for monitoring scripts that poll pipeline health: if .success === false, alert.' },
          { label: 'GET /api/health', detail: 'Comprehensive fleet health: status (healthy/degraded/unhealthy), uptime_seconds, session counts, per-persona health (consecutiveFailures), per-pipeline status (lastSuccess). status=unhealthy if any enabled persona has 3+ consecutive failures; degraded for 1-2. Response cached for 10s (Cache-Control: max-age=10). Integrate with Uptime Robot or any HTTP health check.' },
          ...
          { label: 'CLI — scripts/colony-cli.sh', detail: 'Shell script wrapping the Colony REST API for terminal, CI, and SSH use. Subcommands: status (health + counts), sessions (list all), session <id> (detail + last 20 lines output), pipelines (list), trigger <name> (fire with optional --prompt/--model/--budget overrides), whisper <id> "<msg>" (send message), personas (list), dashboard (print URL). Reads COLONY_API_TOKEN from env or --token flag. COLONY_PORT env sets the port (default 7474). Use --json for machine-readable output.' },
        ],
      },
      {
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Ask a Question', detail: 'Right-click any persona card → "Ask a Question..." to launch a lightweight one-off Q&A session with that persona. The persona\'s full identity file is injected as context, but the normal planning loop is suppressed — no backlogs read, no briefs written, no state updated. The session is named "Q: {Persona Name}" and focused immediately. Cmd+Enter submits, Escape closes. Use this to quickly ask "what did you learn last session?", "what\'s the status of X?", or any question that doesn\'t need a full run.' },
          ...
          { label: 'Dependency Chains', detail: 'Add depends_on: [persona-id, ...] to persona frontmatter to gate this persona on one or more others completing a run first. Example: colony-developer depends_on: [colony-product] means Developer only fires if Product has had a successful run since Developer last ran. Dependency checks are skipped for manual runs and for handoff triggers originating from a dependency. Disabled dependencies are treated as satisfied. The persona card shows a "⤴ Name" badge — amber when any dependency is stale, purple when all are fresh. Dashed amber arrows appear in the Trigger Map between dependency and dependent.' },
          { label: 'Last-Run Stats in Prompts', detail: 'Every persona session now receives a "## Last Run" section in its system prompt: exit code, duration, cost, commits, and aggregate health (success rate, consecutive failures over last 10 runs). If the last session failed (non-zero exit), a prominent note is injected to encourage investigation before proceeding. On first-ever run, the section says "First run — no history." This helps personas adapt — scope down after expensive runs, investigate after failures, and recognize when they are on a losing streak.' },
          { label: 'Prompt Playground', detail: 'Click the Terminal icon on any persona card to open the Prompt Playground. The Role section from the persona\'s identity file appears in an editable textarea. Edit it and click "Test Run" to fire a one-shot claude -p invocation — the model responds to verify its role understanding. "Show Diff" compares edits to the saved version. "Save" writes the edited Role section back to the identity file. "Reset" reverts to the current saved content.', icon: 'Terminal' },
        ],
      },
      {
        name: 'Expanded Card',
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Fan-Out', detail: 'Decompose a large task into N parallel sub-sessions each handling a different part. Click to open the Fan-Out dialog: describe the overall goal, choose 2–6 sub-sessions, let AI decompose it (editable before launch), then launch all at once. All sub-sessions are auto-assigned to a new Space. A status monitor appears in the parent session showing each sub-session\'s progress, cost, and duration. Click any row to navigate to that sub-session.', icon: 'Network' },
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Group by logical concern', detail: 'Toggle the "Group" button (Layers icon) in the header to switch from flat file list to AI-grouped view. Colony sends the file list to Claude Haiku and gets back logical groups (e.g. "Auth middleware", "Tests", "Config"). Files appear under collapsible group headers showing group label, file count, and line delta. Grouping is cached per commit state — no re-analysis on tab switch. Falls back silently to file view if the AI call fails or times out (10s).', icon: 'Layers' },
