...
export const helpContent: Record<string, HelpEntry> = {
  sessions: {
    ...
    zones: [
      ...
      {
        name: 'Context Window',
        position: 'Session status strip (bottom of terminal when session is running)',
        items: [
          ...
          { label: 'Tooltip', detail: 'Hover over the context meter to see a breakdown: system prompt, conversation history, handoff artifacts, and other tracked data. Exact token counts shown as well.' },
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Quota status indicators', detail: 'OK: within limits. Warned: >= warn threshold. Throttled: rate-limited after exceeding threshold (requests delayed). Blocked: >= hard limit (new sessions for blocked team/project are rejected by pipeline-engine).' },
