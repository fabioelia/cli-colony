    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Context budget badge', detail: 'Amber "ctx" badge when a session has generated significant output (context building up). Red when near the limit. Click to open the Handoff Doc export. When amber fires, a .colony-checkpoint.md is also auto-saved to the working directory and injected as preamble on the next session start there.' },
          { label: 'Cost badge', detail: 'Running API cost estimate (e.g. "$0.12") parsed from Claude CLI output. Appears once cost exceeds $0.01. Hover for the exact figure to 4 decimal places.' },
