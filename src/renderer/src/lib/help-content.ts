    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Branch chip', detail: 'Shows the current git branch. Click to open the branch switcher dropdown. Lists all local branches — click any branch to switch to it via `git checkout`. A "↓N" behind-count badge appears when the current branch is behind its upstream. Switching with uncommitted changes shows an error (use Stash first). Fetch button at the top of the dropdown runs `git fetch` to update remote refs.', icon: 'GitBranch' },
          { label: 'Refresh', detail: 'Reload the change list from git.', icon: 'RefreshCw' },
          { label: 'Stash', detail: 'Stash all uncommitted changes (including untracked files) using `git stash push --include-untracked`. Only visible when there are changes. A numbered badge appears when stashes exist — click it to open a dropdown listing each stash with its message and relative date. Each entry has Apply (restore without removing the stash), Pop (restore and remove the stash), and Drop (delete without restoring) actions.', icon: 'Archive' },
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Diff viewer', detail: 'Right pane shows the selected file\'s diff with syntax highlighting (auto-detected). Toggle Unified/Split view in the toolbar. Diff is lazy-loaded and cached — rapid j/k navigation stays fast.' },
