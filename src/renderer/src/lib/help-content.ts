    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Branch chip', detail: 'Shows the current git branch. Click to open the branch switcher dropdown. Lists local branches with ahead/behind counts relative to the current branch (↑N = branch has N commits not in current; ↓N = current has N commits not in branch). Remote branches have merge buttons (cloud icon). Click any branch to switch via `git checkout`. A "↓N" behind-count badge appears when behind upstream. Delete (trash), Merge, and Rebase (GitCommit) buttons per branch row. Prune removes stale remote refs.', icon: 'GitBranch' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Conflict banner', detail: 'When a merge, cherry-pick, or revert encounters conflicts, an amber banner appears at the top of the changes panel. Shows the operation type and count of conflicted files. Once all conflicts are resolved, shows a "Complete" button to run `git commit --no-edit` and finalize the operation. "Abort" cancels the in-progress operation.', icon: 'AlertTriangle' },
