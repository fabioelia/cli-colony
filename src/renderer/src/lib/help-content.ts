    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Branch chip', detail: 'Shows the current git branch. Click to open the branch switcher dropdown. Lists local branches with ahead/behind counts relative to the current branch (↑N = branch has N commits not in current; ↓N = current has N commits not in branch). Remote branches have merge buttons (cloud icon). Click any branch to switch via `git checkout`. A "↓N" behind-count badge appears when behind upstream. Delete (trash), Merge, and Rebase (GitCommit) buttons per branch row. Prune removes stale remote refs. Clicking Merge first shows a preview: file count, +/- stats, fast-forward indicator (⚡), and up to 8 changed files. Confirm with the Merge button or Cancel to dismiss.', icon: 'GitBranch' },
