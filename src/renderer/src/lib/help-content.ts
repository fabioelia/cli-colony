    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Group quick actions', detail: 'Hover a group header and click the ⋯ icon to open a menu of group-scoped actions: Stop All (halts all running sessions in this group), Remove Stopped (removes all non-running sessions from this group), and Send Prompt… (broadcasts a message to all running sessions in the group). Send Prompt opens an inline textarea — Enter to send, Shift+Enter for newline, Esc to cancel.', icon: 'MoreHorizontal' },
          ...
          { label: 'Git branch line', detail: 'Session tiles show the git branch name below the session name when not on main/master. Useful when working across multiple feature branches at once. Hidden on main/master to reduce noise.', icon: 'GitBranch' },
        ],
      },
      {
        name: 'Sidebar Tabs',
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Pre-run hooks', detail: 'Add pre_run: [{type: refresh-prs}] to a pipeline to refresh external data before the action fires. Hooks run sequentially after the condition passes. Currently supported: refresh-prs — fetches latest PRs for all configured repos and rewrites pr-context.md so the session launches with fresh data. Hook failures are logged as warnings but do not block the action. Timing is logged in the debug log.' },
        ],
      },
      {
    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Promote Winner', detail: 'After a winner is declared (manual vote or Auto-Judge), a green banner appears with Reveal and Keep Winner Only buttons. Reveal opens the winner\'s git worktree in Finder. Keep Winner Only removes all loser worktrees while preserving the winner\'s code — you can then use the ReviewPanel to commit and push the winning branch.', icon: 'FolderOpen' },
