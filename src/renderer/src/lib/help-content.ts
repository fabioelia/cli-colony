    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'Add Output Alert', detail: 'Right-click any session → "Add Output Alert…" to define a pattern. Choose Regex mode for advanced matching (case-insensitive). Enable One-shot to receive a single notification and auto-remove the alert. Multiple alerts can be active on the same session simultaneously.', icon: 'AlertCircle' },
        ],
      },
      {
        name: 'Sidebar Tabs',
    ...
    zones: [
      {
        ...
        items: [
          ...
          { label: 'Paste YAML', detail: 'Paste raw YAML text to import a pipeline directly — no file needed. Click the Copy icon in the header, paste your YAML, and click Import. The pipeline name is read from the "name:" field in the YAML. Useful for sharing pipelines via chat or documentation.', icon: 'Copy' },
