    ...
    zones: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: 'on_failure config', detail: 'Add on_failure to any action to control failure behavior. Supports three keys: notify: true sends a desktop notification when the stage fails (gated by the pipeline notification level); run: "action-name" fires a named action from the pipeline action tree as a fire-and-forget recovery step (cost added to run total); retry: {max: N} retries the action up to N times with the last 20 lines of the failed session output injected as context (so the retried session can avoid repeating the same mistake). The stage history records retryCount and a retryContext flag when context was injected. Regardless of on_failure config, any stage failure triggers a desktop notification when the pipeline notification level is Failures or All.' },
