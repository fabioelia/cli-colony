    ...
    <div className="settings-section">
      ...
      {isExpanded && (
        <div className="settings-section-content">
          {error && (
            <div className="settings-error" style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', color: '#ef4444', fontSize: '12px' }}>
          ...
          {!editMode ? (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Schedule</label>
                  <div style={{ fontSize: '13px', fontFamily: 'monospace' }}>{config.schedule}</div>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Concurrency</label>
                  <div style={{ fontSize: '14px' }}>{config.concurrency} task{config.concurrency !== 1 ? 's' : ''} parallel</div>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Timeout</label>
                  <div style={{ fontSize: '14px' }}>{config.timeoutPerTaskMinutes} min per task</div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>On Completion</label>
            ...
            <>
              ...
              <div style={{ display: 'grid', gap: '12px', marginBottom: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Cron Schedule</label>
                  <input
                    type="text"
                  ...
                  />
                  <div style={{ fontSize: '11px', color: '#888', marginTop: '2px' }}>Example: "0 2 * * *" = 2am daily</div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Concurrency (1–5)</label>
                    <input
                      type="number"
                      min="1"
                      max="5"
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Timeout (min)</label>
                    <input
                      type="number"
                      min="1"
                ...
                {formData.onCompletion === 'report' && (
                  <div>
                    <label style={{ display: 'block', fontSize: '12px', color: '#888', marginBottom: '4px' }}>Report Recipients</label>
                    <input
                      type="text"
          ...
          {history.length > 0 && (
            <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #333' }}>
              <h4 style={{ fontSize: '12px', color: '#888', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
